...
<span>项目名称：</span><input type="text" id="xiangmumingcheng" name="xiangmumingcheng" value="${entity.xiangmumingcheng}"/>
    <span>卡号：</span><input type="text" id="kahao" name="kahao" value="${entity.kahao}"/>
...
<input type="submit" id="btnSearch" value="查询"  class="templatemo-blue-button" />
 ...
 <table id="gvUser" class="table table-striped table-bordered templatemo-user-table" cellpadding="0">
            <tr>
			 <th>选择</th>
           <th>项目名称</th><th>卡号</th><th>密码</th><th>状态</th><th>金额</th>
            </tr>
<c:forEach items="${list}" var="item">
<tr>
		   <td><input type="checkbox" name="checkuser" value="${item.id}" /> </td>
          
		  <td> ${item.xiangmumingcheng}  </td><td> ${item.kahao}  </td><td> ${item.mima}  </td><td> ${item.zhuangtai}  </td><td> ${item.jine}  </td>
</tr>
</c:forEach>
             </table>
 </div>
