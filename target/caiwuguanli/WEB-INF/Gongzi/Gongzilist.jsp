...
<head>
    ...
    <script type="text/javascript">
        ...
        function Add() {
            yuefen = $("#yuefen").val();
            $.post("Edit.action",{yuefen:yuefen},function(data){
                window.location.href="list.action?yuefen="+yuefen;
            });
        }
                                ...
                                <div class="templatemo-login-form">
                                    <div class="form-group">
                                        <form action="list.action" method="post">
                                            ...
                                            <span>月份：</span><input type="text" id="yuefen" name="yuefen"
                                                                     value="${entity.yuefen}" class="Wdate"
                                                                     onfocus="WdatePicker({dateFmt:'yyyy-MM',readOnly:true})" style="height:34px;"/>
                                            <span>工号：</span><input type="text" id="gonghao" name="gonghao"
                                                                     value="${entity.gonghao}"/>
                                            <span>姓名：</span><input type="text" id="xingming" name="xingming"
                                                                     value="${entity.xingming}"/>

                                            <input type="submit" id="btnSearch" value="查询"
                                                   class="templatemo-blue-button"/>
                                        </form>
                                        <input type="button" id="btnAdd" value="计算工资" onclick="Add();"
                                               class="templatemo-blue-button"/>&nbsp;
                        ...
                        <table id="gvUser" class="table table-striped table-bordered templatemo-user-table"
                            ...
                            <c:forEach items="${list}" var="item">
                                <tr>
                                    ...
                                    <td> ${item.gonghao} </td>
                                    <td> ${item.xingming} </td>
                                    <td> ${item.jibengongzi} </td>
                                    <td> ${item.gongjijingerenjine} </td>
                                    <td> ${item.gongjijinzongjine} </td>
                                    <td> ${item.shifagongzi} </td>
                                </tr>
                            </c:forEach>
                        </table>
                    </div>
