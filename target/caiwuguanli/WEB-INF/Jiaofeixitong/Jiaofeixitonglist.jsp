...
<span>缴费项目：</span><input type="text" id="jiaofeixiangmu" name="jiaofeixiangmu" value="${entity.jiaofeixiangmu}"/>
    <span>用户：</span><input type="text" id="yonghuid" name="yonghuid" value="${entity.yonghuid}"/>

    <span>状态：</span><input type="text" id="zhuangtai" name="zhuangtai" value="${entity.zhuangtai}"/>
<input type="submit" id="btnSearch" value="查询"  class="templatemo-blue-button" />
 ...
 <table id="gvUser" class="table table-striped table-bordered templatemo-user-table" cellpadding="0">
            <tr>
			 <th>选择</th>
           <th>缴费项目</th><th>备注</th><th>用户</th><th>缴费金额</th><th>状态</th>
            </tr>
<c:forEach items="${list}" var="item">
<tr>
		   <td><input type="checkbox" name="checkuser" value="${item.id}" /> </td>
          
		  <td> ${item.jiaofeixiangmu}  </td><td> ${item.beizhu}  </td><td> ${item.name}  </td><td> ${item.jiaofeijine}  </td><td> ${item.zhuangtai}  </td>
</tr>
</c:forEach>
             </table>
 </div>
